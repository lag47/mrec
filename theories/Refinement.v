...
Instance Proper_eutt_satisfies {E R1 R2} RR :
  Proper (eutt eq ==> eutt eq ==> iff) (@satisfies E R1 R2 RR).
Proof.
  repeat intro.
Abort.
...
Inductive refinesF {E1 E2 R1 R2} (RE : forall (A B : Type), E1 A -> E2 B -> Prop ) (RR : R1 -> R2 -> Prop) (F : itree_spec E1 R1 -> itree_spec E2 R2 -> Prop) : 
  itree_spec' E1 R1 -> itree_spec' E2 R2 -> Prop :=
  | refines_Ret r1 r2 : RR r1 r2 -> refinesF RE RR F (RetF r1) (RetF r2)
  | refines_TauLR phi1 phi2 :
      F phi1 phi2 -> refinesF RE RR F (TauF phi1) (TauF phi2)
(*
...
*)
  | refines_VisLR A B e1 e2 kphi1 kphi2 :
      RE A B e1 e2 -> (* need some notion of RAns, possibly stuffed into *)
      (forall a b, F (kphi1 a) (kphi2 b) ) ->
      refinesF RE RR F (VisF (Spec_vis e1) kphi1) (VisF (Spec_vis e2) kphi2)
  | refines_VisLRForall A B kphi1 kphi2 :
      (forall (a : A) (b : B), F (kphi1 a) (kphi2 b) ) ->
      refinesF RE RR F (VisF Spec_forall kphi1) (VisF Spec_forall kphi2)
  | refines_VisLRExists A B kphi1 kphi2 :
      (forall (a : A) (b : B), F (kphi1 a) (kphi2 b) ) ->
      refinesF RE RR F (VisF Spec_exists kphi1) (VisF Spec_exists kphi2)
  | refines_forallR A kphi phi :
      (forall a : A, refinesF RE RR F phi (observe (kphi a))) ->
      refinesF RE RR F phi (VisF Spec_forall kphi)
  | refines_forallL A kphi phi (a : A) :
      (refinesF RE RR F (observe (kphi a)) phi) ->
      refinesF RE RR F (VisF Spec_forall kphi) phi
  | refines_existsR A kphi phi (a : A) :
      (refinesF RE RR F phi (observe (kphi a))) ->
      refinesF RE RR F phi (VisF Spec_exists kphi)
  | refines_existsL A kphi phi :
      (forall a : A, refinesF RE RR F (observe (kphi a)) phi) ->
      refinesF RE RR F (VisF Spec_exists kphi) phi
.
Hint Constructors refinesF.
Definition refines_ {E1 E2 R1 R2} RE RR F (t1 : itree_spec E1 R1) (t2 : itree_spec E2 R2) : Prop :=
  refinesF RE RR F (observe t1) (observe t2).

Lemma monotone_refinesF {E1 E2 R1 R2} RE RR ot1 ot2 sim sim' 
  (LE : sim <2= sim' )
  (IN : @refinesF E1 E2 R1 R2 RE RR sim ot1 ot2) : 
  refinesF RE RR sim' ot1 ot2.
Proof.
  induction IN; eauto.
Qed.

Lemma monotone_refines_ {E1 E2 R1 R2} RE RR : monotone2 (@refines_ E1 E2 R1 R2 RE RR).
...
Definition refines {E1 E2 R1 R2} RE RR : itree_spec E1 R1 -> itree_spec E2 R2 -> Prop :=
  paco2 (refines_ RE RR) bot2.
(*
...
Instance Reflexive_refines {E R} RE RR : ReflexiveE RE -> Reflexive RR -> Reflexive (@refines E E R R RE RR).
Proof.
  red. pcofix CIH. intros HRR HRE t. pfold. red.
...
Lemma refines_Vis_forallR : forall (E : Type -> Type) (R1 R2 A : Type) RR (t : itree_spec E R1) (k : A -> itree_spec E R2),
         refines RR t (Vis Spec_forall k) ->
         forall a : A, refines RR t (k a).
Proof.
  intros E R1 R2 A RR. pcofix CIH. intros t k Href a.
  ...
  - econstructor. Unshelve. 2 : apply a. specialize (H a).
    assert (paco2 (refines_ RR) r (kphi1 a) (k a)). eapply paco2_mon; intros; try contradiction; eauto.
    inv PR. punfold H0.
  - clear H0. assert (refines RR (go phi) (k a) ).
    { pstep. apply H. }
    enough (paco2 (refines_ RR) r (go phi) (k a) ).
    { punfold H1. }
...
Lemma refines_Vis_existsL : forall (E : Type -> Type) (R1 R2 A : Type) RR (t : itree_spec E R1) (k : A -> itree_spec E R2),
         refines RR (Vis Spec_exists k) t ->
         forall a : A, refines RR (k a) t.
Proof.
  intros E R1 R2 A RR. intros t k Href.
...
Inductive forallRefinesF {E R1 R2} (RR : R1 -> R2 -> Prop) (F : itree_spec E R1 -> itree_spec E R2 -> Prop)
          {A} (kphi1: A -> itree_spec E R1)
  : itree_spec' E R2 -> Prop :=
  | forallRefines_VisLR kphi2 :
      (forall a : A, F (kphi1 a) (kphi2 a)) ->
      forallRefinesF RR F kphi1 (VisF Spec_forall kphi2)
  | forallRefines_forallR B kphi2 :
      (forall b : B, forallRefinesF RR F kphi1 (observe (kphi2 b))) ->
      forallRefinesF RR F kphi1 (VisF Spec_forall kphi2)
  | forallRefines_forallL phi (a : A) :
      refinesF RR F (observe (kphi1 a)) phi ->
      forallRefinesF RR F kphi1 phi
  | forallRefines_existsR B kphi2 (b : B) :
      (forallRefinesF RR F kphi1 (observe (kphi2 b))) ->
      forallRefinesF RR F kphi1 (VisF Spec_exists kphi2)
.

(* FIXME: should we replace the recursive call to refinesF in the above with
just a refines? *)

Lemma refinesF_Vis_forallL : forall (E : Type -> Type) (R1 R2 A : Type) RR F
                                   (t : itree_spec' E R2) (k : A -> itree_spec E R1),
    refinesF RR F (VisF Spec_forall k) t ->
    @forallRefinesF E R1 R2 RR F A k t.
Proof.
...
Inductive tauRefinesF {E R1 R2} (RR : R1 -> R2 -> Prop) (F : itree_spec E R1 -> itree_spec E R2 -> Prop)
          (phi1: itree_spec E R1)
  : itree_spec' E R2 -> Prop :=
...
Lemma refinesF_Tau : forall (E : Type -> Type) (R1 R2 : Type) RR F
                            t1 (t2 : itree_spec' E R2),
    refinesF RR F (TauF t1) t2 ->
    @tauRefinesF E R1 R2 RR F t1 t2.
Proof.
...
Theorem concreteRefinesTrans {E R1 R2 R3} (RR1 : R1 -> R2 -> Prop) (RR2 : R2 -> R3 -> Prop) t1 t2 t3
         (isc:isConcrete t1) :
  @refines E _ _ RR1 t1 t2 -> refines RR2 t2 t3 -> refines (rcompose RR1 RR2) t1 t3.
Proof.
...
Instance eq_itree_refines_Proper1 {E R1 R2 RR r} :  Proper (eq_itree eq ==> eq_itree eq ==> flip impl) 
                                                           (@refines_ E R1 R2 RR (upaco2 (refines_ RR) r)).
Admitted. (* needs an axiom to prove this *)

Instance eq_itree_refines_Proper2 {E R1 R2 RR r} :  Proper (eq_itree eq ==> eq_itree eq ==> flip impl) 
                                                           (paco2 (@refines_ E R1 R2 RR) r).
Admitted.

Theorem refines_bind {E R1 R2 S1 S2} RR RS
        (t1 : itree_spec E R1) (t2 : itree_spec E R2)
        (k1 : R1 -> itree_spec E S1)
        (k2 : R2 -> itree_spec E S2) :
  refines RR t1 t2 -> (forall r1 r2, RR r1 r2 -> refines RS (k1 r1) (k2 r2)) ->
  refines RS (ITree.bind t1 k1) (ITree.bind t2 k2).
Proof.
  ...
  - pclearbot. rewrite Heqot1, Heqot2. repeat rewrite bind_vis.
    pstep. constructor. intros. right. eapply CIH; eauto. apply H.
...
Lemma refines_iter_bind_aux:
  forall (E : Type -> Type) (S2 S1 R1 R2 : Type) 
    (RR : R1 -> R2 -> Prop) (RS : S1 -> S2 -> Prop)
    (k1 : R1 -> itree_spec E (R1 + S1))
    (k2 : R2 -> itree_spec E (R2 + S2))
    (r : itree_spec E S1 -> itree_spec E S2 -> Prop),
    (forall (r1 : R1) (r2 : R2),
        RR r1 r2 -> r (ITree.iter k1 r1) (ITree.iter k2 r2)) ->
    forall (phi1 : itree_spec E (R1 + S1)) (phi2 : itree_spec E (R2 + S2)),
      refines (HeterogeneousRelations.sum_rel RR RS) phi1 phi2 ->
      paco2 (refines_ RS) r
            (phi1 >>=
                  (fun lr : R1 + S1 =>
                     match lr with
...
Proof.
  intros E S2 S1 R1 R2 RR RS k1 k2 r CIH.
  ...
  - setoid_rewrite bind_vis. pstep. constructor. right. eapply CIH'; eauto. apply H.
...
Theorem refines_iter {E R1 R2 S1 S2} (RR : R1 -> R2 -> Prop) (RS : S1 -> S2 -> Prop)
        (k1 : R1 -> itree_spec E (R1 + S1)) (k2 : R2 -> itree_spec E (R2 + S2)) :
  (forall r1 r2, RR r1 r2 -> refines (HeterogeneousRelations.sum_rel RR RS) (k1 r1) (k2 r2) ) ->
  forall r1 r2, RR r1 r2 ->  refines RS (ITree.iter k1 r1) (ITree.iter k2 r2).
Proof.
  ...
  - setoid_rewrite bind_vis. pstep. constructor. left. pclearbot.
